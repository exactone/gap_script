...
swap_option(){
	echo "******************************"
	echo "please enter the size of swap(MB). n/N for cancel. (請輸入swap的大小(MB). 若要取消請輸入 n/N)"
	echo "******************************"
	read SWAPSIZE
	if [ grep '^[[:digit:]]*$' <<< "$SWAPSIZE" ]; then
		echo "swap set to $SWAPSIZE MB. (swap大小設定為 $SWAPSIZE MB)"
		ADDSWAP='y'

	elif [ "$SWAPSIZE" == "n" -o "$SWAPSIZE" == "N"]; then
		SWAPSIZE='0'
		ADDSWAP='n'
	else
		echo "Illegal setting, skip swap setting. (錯誤的設定，將略過swap設定)"
		SWAPSIZE='0'
		ADDSWAP='n'
	fi
...
if [ "$TF_GPU_CPU" == "tensorflow-gpu" ]; then
	##########
	echo_progress "install cuda 9.0 for TensorFlow 1.8" "(安裝cuda 9.0, TensorFlow 1.8支援cuda 9.0)"
	##########
	mkdir ~/download
	cd ~/download
	...
	##########
	echo_progress "install cudnn 7 for TensorFlow 1.8" "(安裝cudnn 7, cudnn 7支援cuda 9.0)"
	##########
	cd ~/download
	...
	wget -O libcudnn7_7.1.3.16-1+cuda9.0_amd64.deb "https://www.dropbox.com/s/r56pcrmux2rxdrc/libcudnn7_7.1.3.16-1%2Bcuda9.0_amd64.deb?dl=0"
	yes Y | sudo dpkg -i libcudnn7_7.1.3.16-1+cuda9.0_amd64.deb
...
##########
echo_progress "install XGboost 0.71 from conda-forge" "(從conda-forge安裝 XGboost 0.71)"
##########
